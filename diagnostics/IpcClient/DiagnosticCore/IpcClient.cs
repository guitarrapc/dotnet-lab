...
namespace DiagnosticCore
{
    internal class IpcClient
    {
        ...
        public static string DiagnosticsPortPattern { get; } = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? $@"^{IpcSettings.PipeName}(\d+)$" : $@"^{IpcSettings.PipeName}(\d+)-(\d+)-socket$";
        ...
        private static Stream GetTransport(int processId)
        {
            ...
            {
                var pipeName = $"{IpcSettings.PipeName}{processId}";
            ...
            else
            {
                ...
                try
                {
                    ipcPort = Directory.GetFiles(IpcRootPath, $"{IpcSettings.PipeName}{processId}-*-socket") // Try best match.
