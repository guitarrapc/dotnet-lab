        ...
        private class SampleServer : IDisposable
        {
            ...
            public void Start(string url)
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    // backend
                    using var dbSource = new ActivitySource("Samples.SampleServer.Db");
                    using var redisSource = new ActivitySource("Samples.SampleServer.Redis");
                    ...
                    while (this.listener.IsListening)
                    {
                        try
                        {
                            ...
                            // backend
                            using (var redisSpan = redisSource.StartActivity("redis", ActivityKind.Internal))
                            {
                                redisSpan.SetTag("key", "uid-123");
                                redisSpan.SetTag("status", "up");
                                await Task.Delay(TimeSpan.FromMilliseconds(1));
                            }
                            using (var dbSpan = dbSource.StartActivity("db", ActivityKind.Internal))
                            {
                                dbSpan.SetTag("database", "user");
                                dbSpan.SetTag("table", "status");
                                dbSpan.SetTag("uid", "123");
                                await Task.Delay(TimeSpan.FromMilliseconds(traceRandom.Next(2, 10)));
                            }
